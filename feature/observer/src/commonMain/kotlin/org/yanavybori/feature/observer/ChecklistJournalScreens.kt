...
internal fun ChecklistScreen(
    ...
    val problemMediaPicker = LocalPlatformUi.current.rememberMediaPicker { uri ->
...
internal fun JournalScreen(
    ...
    val exportLauncher = platform.rememberDocumentCreator { uri ->
        if (uri != null) {
            coroutineScope.launch {
                val result = runCatching {
                    val bytes = withContext(Dispatchers.Default) {
                        buildJournalCsv(session, dayTitles, events).encodeToByteArray()
                    }
    ...
    LazyColumn(
        ...
        item {
            OutlinedButton(
                onClick = {
                    exportStatus = null
                    exportLauncher.launch(DocumentRequest(journalExportFileName(session, org.yanavybori.core.common.SystemClock.now()), "text/csv"))
                },
...
internal fun NewJournalEventFlow(
    ...
    val platform = LocalPlatformUi.current
    val mediaPicker = LocalPlatformUi.current.rememberMediaPicker { uri ->
        if (uri != null) {
            viewModel.importMedia(uri, MediaSource.PHOTO_PICKER) { asset ->
