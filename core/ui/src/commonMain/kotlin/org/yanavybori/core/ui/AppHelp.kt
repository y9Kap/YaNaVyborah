    ...
    if (opened && !fullGuide) AlertDialog(
        ...
        text = {
            Column(Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                Text("Откройте «Наблюдатель», укажите регион и номер УИК, создайте сессию и запомните пароль удаления. ФИО и остальные сведения можно добавить по желанию.")
                ...
                Text("Документы доступны офлайн. В «Избирателе» поиск запускается кнопкой: по умолчанию УмГ имеет приоритет, а другие списки дополняют пропуски. Режим можно изменить в свёрнутых настройках. Записи сохраняются на устройстве; приложение ничего не отправляет автоматически.")
            }
        },
...
private fun AppGuideDialog(onBack: () -> Unit) {
    ...
    val sections by androidx.compose.runtime.produceState<List<Pair<String, String>>>(emptyList(), platform) {
        value = platform.readBundledFile("app-guide.md").decodeToString()
